    ...
    if chat.type != chat.PRIVATE:
        if len(args) >= 1:
            ...
            if (var == "no"):
                ...
                update.effective_message.reply_text("Conexões desativadas para este chat para usuários")
            elif(var == "yes"):
                sql.set_allow_connect_to_chat(chat.id, True)
                update.effective_message.reply_text("Conexões habilitadas para este chat para usuários")
            else:
    ...
    if update.effective_chat.type == 'private':
        if len(args) >= 1:
            try:
                ...
                update.effective_message.reply_text("ID de bate-papo inválido fornecido!")
            if (bot.get_chat_member(connect_chat, update.effective_message.from_user.id).status in ('administrator', 'creator') or 
                ...
                if connection_status:
                    ...
                    update.effective_message.reply_text("A conexão falhou!")
            else:
                update.effective_message.reply_text("Conexões para este bate-papo não permitidas!")
        else:
...
def disconnect_chat(bot, update):
    if update.effective_chat.type == 'private':
        ...
        if disconnection_status:
           ...
           update.effective_message.reply_text("Desconexão malsucedida!")
    else:
        ...
        if (bot.get_chat_member(conn_id, user_id).status in ('administrator', 'creator') or 
            ...
            if need_admin == True:
                if bot.get_chat_member(conn_id, update.effective_message.from_user.id).status in ('administrator', 'creator') or user_id in SUDO_USERS:
                    ...
                    update.effective_message.reply_text("Você precisa ser um administrador em um grupo conectado!")
                    exit(1)
            else:
                return conn_id
        else:
            update.effective_message.reply_text("Grupo mudou de conexão de direitos ou você não é mais administrador.\nVou desconectar você.")
...
__help__ = """
Ações estão disponíveis com grupos conectados:
 â€¢ Ver e editar notas
 â€¢ Ver e editar filtros
 â€¢ Mais no futuro!

 - /connect <chatid>: Conecte-se ao bate-papo remoto
 - /disconnect: Desconectar do bate-papo
 - /allowconnect on/yes/off/no: Permitir conectar usuários ao grupo
"""

__mod_name__ = "ðŸ”— Conexões ðŸ”—"
