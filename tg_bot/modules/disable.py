...
if is_module_loaded(FILENAME):
    ...
    def disable(bot: Bot, update: Update, args: List[str]):
        ...
        if len(args) >= 1:
            ...
            if disable_cmd in set(DISABLE_CMDS + DISABLE_OTHER):
                ...
                update.effective_message.reply_text("Desativou o uso de `{}`".format(disable_cmd),
                                                    parse_mode=ParseMode.MARKDOWN)
            else:
                update.effective_message.reply_text("Esse comando não pode ser desativado")

        else:
            update.effective_message.reply_text("O que devo desabilitar?")
    ...
    def enable(bot: Bot, update: Update, args: List[str]):
        ...
        if len(args) >= 1:
            ...
            if sql.enable_command(chat.id, enable_cmd):
                update.effective_message.reply_text("Habilitou o uso de `{}`".format(enable_cmd),
                                                    parse_mode=ParseMode.MARKDOWN)
            else:
                update.effective_message.reply_text("Isso está desativado?")

        else:
            update.effective_message.reply_text("O que devo habilitar?")
    ...
    def list_cmds(bot: Bot, update: Update):
        if DISABLE_CMDS + DISABLE_OTHER:
            ...
            update.effective_message.reply_text("Os seguintes comandos podem ser alternados:\n{}".format(result),
                                                parse_mode=ParseMode.MARKDOWN)
        else:
            update.effective_message.reply_text("Nenhum comando pode ser desativado.")
    ...
    def build_curr_disabled(chat_id: Union[str, int]) -> str:
        ...
        if not disabled:
            return "Nenhum comando está desabilitado!"

        result = ""
        for cmd in disabled:
            result += " - `{}`\n".format(escape_markdown(cmd))
        return "Os seguintes comandos são atualmente restritos:\n{}".format(result)
    ...
    def __stats__():
        return "{} itens desabilitados, em {} chats.".format(sql.num_disabled(), sql.num_chats())
    ...
    __mod_name__ = "ðŸªš Cmds Desligado ðŸªš"

    __help__ = """
 - /cmds: verifique o estado atual dos comandos desabilitados

*Admin only:*
 - /enable <cmd nome>: habilite aquele comando
 - /disable <cmd nome>: desabilite aquele comando
 - /listcmds: lista todos os comandos alternáveis possíveis
    """
