    ...
    if can_delete(chat, bot.id):
        if len(args) >= 1:
            if args[0] in LOCK_TYPES:
                ...
                message.reply_text("Trancado {} mensagens para todos os não administradores!".format(args[0]))
                ...
                message.reply_text("Trancado {} para todos os não administradores!".format(args[0]))
                return "<b>{}:</b>" \
                       "\n#TRANCAR" \
                       "\n<b>Admin:</b> {}" \
                       "\nLocked <code>{}</code>.".format(html.escape(chat.title),
                ...
                message.reply_text("O que você está tentando bloquear ...? Tente / locktypes para a lista de bloqueáveis")

    else:
        message.reply_text("Não sou um administrador ou não tenho direitos de exclusão.")
    ...
    if is_user_admin(chat, message.from_user.id):
        if len(args) >= 1:
            if args[0] in LOCK_TYPES:
                ...
                message.reply_text("O que você está tentando desbloquear ...? Tente / locktypes para a lista de bloqueáveis")

        else:
            bot.sendMessage(chat.id, "O que você está tentando desbloquear ...?")
                ...
                for new_mem in new_members:
                    if new_mem.is_bot:
                        if not is_bot_admin(chat, bot.id):
                            message.reply_text("Eu vejo um bot, e me disseram para impedi-los de entrar... "
                                               "mas eu não sou administrador!")
                            return

                        chat.kick_member(new_mem.id)
                        message.reply_text("Apenas administradores podem adicionar bots a este chat! Saia daqui.")
            else:
                try:
                    message.delete()
                    ...
                    if excp.message == "Mensagem para deletar não encontrada":
                        ...
                        LOGGER.exception("ERROR em bloqueáveis")
        ...
        if filter(msg) and sql.is_restr_locked(chat.id, restriction) and can_delete(chat, bot.id):
            try:
                ...
                if excp.message == "Mensagem para deletar não encontrada":
                    ...
                    LOGGER.exception("ERROR nas restrições")
            break
...
def build_lock_message(chat_id):
    ...
    if not (locks or restr):
        res = "Não há bloqueios atuais neste bate-papo."
    else:
        res = "Estas são as fechaduras neste chat:"
        if locks:
            res += "\n - sticker = `{}`" \
                   "\n - audio = `{}`" \
                   "\n - voice = `{}`" \
...
__help__ = """
 - /locktypes: uma lista de possíveis tipos de bloqueio

*Apenas administrador:*
 - /lock <type>: itens de bloqueio de um determinado tipo (não disponível em privado)
 - /unlock <type>: desbloquear itens de um determinado tipo (não disponível em privado)
 - /locks: a lista atual de bloqueios neste chat.

Os bloqueios podem ser usados ​​para restringir os usuários de um grupo.
por exemplo:
Bloquear urls excluirá automaticamente todas as mensagens com URLs que não foram colocados na lista de permissões, bloquear adesivos excluirá todas \
adesivos, etc.
Bloquear bots impedirá que não administradores adicionem bots ao chat.
"""

__mod_name__ = "🔐 Fechaduras 🔐"
